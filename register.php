...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($select_users) > 0) {
        ...
        if ($pass != $cpass) {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO `users` (name, birthplace, email, no_hp, address, password, question_forgot, answer_forgot) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssssss", $name, $birthplace, $email, $no_hp, $address, $pass, $question, $answer);
...
<section class="form-container">
    <form action="" method="post">
        ...
        <input type="text" name="birthplace" class="box" placeholder="Tempat Tanggal Lahir: " required>
